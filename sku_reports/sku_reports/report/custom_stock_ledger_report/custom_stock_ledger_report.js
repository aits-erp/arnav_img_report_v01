...
//         // 🔽 Gross Weight Coloring (ADDED)
//         if (column.fieldname == "gross_weight_out" && data && data.gross_weight_out > 0) {
//             value = "<span style='color:red'>" + value + "</span>";
//         }

//         if (column.fieldname == "gross_weight_in" && data && data.gross_weight_in > 0) {
//             value = "<span style='color:green'>" + value + "</span>";
//         }
...
frappe.query_reports["Custom Stock Ledger Report"] = {
	filters: [
		...
		{
			// NEW: Filter by SKU name directly
			fieldname: "sku",
			label: __("SKU"),
			fieldtype: "Link",
			options: "SKU",
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		// SKU-based Stock In / Out / Balance coloring
		if (column.fieldname == "sku_stock_out" && data && data.sku_stock_out < 0) {
			value = "<span style='color:red'>" + value + "</span>";
		} else if (column.fieldname == "sku_stock_in" && data && data.sku_stock_in > 0) {
			value = "<span style='color:green'>" + value + "</span>";
		}

		// Gross Weight Coloring
		if (column.fieldname == "gross_weight_out" && data && data.gross_weight_out > 0) {
			value = "<span style='color:red'>" + value + "</span>";
		}
		if (column.fieldname == "gross_weight_in" && data && data.gross_weight_in > 0) {
